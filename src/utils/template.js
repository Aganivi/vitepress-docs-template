...
export const loadTemplateMap = async () => {
	...
	Object.keys(filesMap).forEach((templateKey) => {
		filesMap[templateKey] &&
			Object.keys(filesMap[templateKey]).forEach((fileKey) => {
				let src = getFileContent(
					getTemplate(templateKey),
					filesMap[templateKey][fileKey]
				)
...
const getReplacementsMap = (content) => {
	...
	const matches = content
		.match(regex)
		.map((match) => match.replace(/\{template ([\s\S]*)%\}/, ($1, $2) => $2))
	const map = {}
...
const getFileContent = (template, file) => {
	...
	return template.replace(
		/\{template ([\s\S]*?)%%\}/g,
		(match, key) => replacementsMap[key] || match
	)
}
